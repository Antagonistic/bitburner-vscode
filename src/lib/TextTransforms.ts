...
interface ITextTransformsMap
{
    [key: string]: ITextTransform
}
...
export class TextTransforms {
	...
	static Highlight: ITextTransformsMap = {
	...
	static Color: ITextTransformsMap = {
	...
	static Transform: ITextTransformsMap = {
	...
	*/
	static apply(text: string, transforms: ITextTransform[]): string {
		const prefix = [];
		const code = [];
		let apply = '';
